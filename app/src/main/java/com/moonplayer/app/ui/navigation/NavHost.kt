...
sealed class Screen(val route: String) {
    ...
    data object ArtistDetail : Screen("artist/{name}") {
        fun create(name: String) = "artist/$name"
    }
    ...
    Scaffold(
        bottomBar = {
            Column {
                ...
                if (currentRoute in listOf(Screen.Library.route, Screen.Settings.route, Screen.Search.route)) {
                    NavigationBar {
                        NavigationBarItem(
                            selected = currentRoute == Screen.Library.route,
                            onClick = { navController.navigate(Screen.Library.route) { popUpTo(Screen.Library.route) { inclusive = true } } },
                            icon = { Icon(if (currentRoute == Screen.Library.route) Icons.Filled.LibraryMusic else Icons.Outlined.LibraryMusic, null) },
                            label = { Text("Biblioteca") }
                        )
                        NavigationBarItem(
                            selected = currentRoute == Screen.Search.route,
                            onClick = { navController.navigate(Screen.Search.route) },
                            icon = { Icon(if (currentRoute == Screen.Search.route) Icons.Filled.Search else Icons.Outlined.Search, null) },
                            label = { Text("Buscar") }
                        )
                        NavigationBarItem(
                            selected = currentRoute == Screen.Settings.route,
                            onClick = { navController.navigate(Screen.Settings.route) },
                            icon = { Icon(if (currentRoute == Screen.Settings.route) Icons.Filled.Settings else Icons.Outlined.Settings, null) },
                            label = { Text("Ajustes") }
                        )
                    }
                }
        ...
        NavHost(
            ...
            composable(
                ...
                ArtistDetailScreen(vm = vm, artist = name, onBack = { navController.popBackStack() }, onSongClick = { s, l -> vm.playSong(s, l) })
            }
            composable(
                ...
                AlbumDetailScreen(vm = vm, albumId = id, onBack = { navController.popBackStack() }, onSongClick = { s, l -> vm.playSong(s, l) })
            }
            composable(
                ...
                val id = entry.arguments?.getLong("id") ?: return@composable
                PlaylistDetailScreen(vm = vm, playlistId = id, onBack = { navController.popBackStack() }, onSongClick = { s, l -> vm.playSong(s, l) })
            }
        }
    }
}
